...
class Api::V1::EventsController < ApplicationController
  ...
  def show
    render json: @event, serializer: EventSerializer, include: [:primary_event_location], status: :ok
  end
  ...
  def create
    ...
    render json: event, serializer: EventSerializer, include: [:primary_event_location], status: :created
  ...
  def event_params
    params.require(:event).permit(
      ...
      :event_date,
      :event_time,
      :status,                # FE sends "planning" | "active" | ...
      :description,
      ...
      location: [
        ...
        :image_url,        # transient; used to download the image
        :photo_reference   # preferred; used to build a fresh photo URL
      ]
    )
  end

  # Minimal policy invoker without full Pundit
